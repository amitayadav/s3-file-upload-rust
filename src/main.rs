...
{
    let provider = ProfileProvider::with_configuration(Path::new("/home/knoldus/.aws/"), "amitayadav");
    let client = S3Client::new(default_tls_client().unwrap(), provider, Region::ApSouth1);
    let file = file.clone();

    print!("{} \"{}\" to \"{}/{}\"...", Yellow.paint("Uploading"), file.path.to_str().unwrap(),
           bucket_name, local_file_get_relative_path(&file, &local_path));
    Some(std::io::stdout().flush());

   let mut file_handle = File::open(&file.path).expect("file couldn't open");
    ...
    match file_handle.read_to_end(&mut contents)
    {
        Ok(_) =>
        {
            ...
            match client.put_object(&request)
            {
                ...
                Err(error) =>
                {
                    println!(" 55{}: {}", Red.paint("Error"), error);
                }
            }
        }
        Err(error) =>
        {
            println!("56{}: {}", Red.paint("Error"), error);
        }
    }
}
...
fn main()
{
    ...
    if !Path::new("/home/knoldus/.aws/").exists()
    {
        println!("{}", Red.paint("credentials file could not be found"));
        return;
    }
    
    let mut ignored_directories: Vec<String> = Vec::new();println!("6");
    ...
    if modified_files.len() > 0 || new_files.len() > 0
    {
        ...
        if confirm_upload
        {
            ...
            let new_checksums = local_file_create_checksums(&files, &local_path);println!("80");
